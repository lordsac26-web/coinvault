...
const tips = [
  ...
  },
  {
    ...
    dos: [
      ...
      'Use the rear camera \u2014 much sharper than the selfie camera',
    ],
    donts: [
    ...
    <div className="rounded-2xl border border-[#c9a84c]/20 overflow-hidden" style={{ background: 'rgba(201,168,76,0.03)' }}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#c9a84c]/15">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-sm font-semibold text-[#e8c97a]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>Coin Photography Tips</h4>
        </div>
        {onClose && <button onClick={onClose} className="text-[#f5f0e8]/30 hover:text-[#f5f0e8]/60 transition-colors"><X className="w-4 h-4" /></button>}
      </div>

      <div className="px-4 py-3 border-b border-[#c9a84c]/10">
        <div className="flex items-start gap-2">
          <Lightbulb className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
          <p className="text-xs text-[#f5f0e8]/60 leading-relaxed">Better photos = better AI grading results. Diffused light, steady hands, and a dark background make the biggest difference.</p>
        </div>
      </div>
      ...
      <div className="divide-y divide-[#c9a84c]/10">
        {tips.map(({ icon: Icon, title, key, dos, donts }) => {
          ...
          return (
            <div key={key}>
              <button onClick={() => toggle(key)} className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-white/[0.02] transition-colors">
              ...
              {isOpen && (
                <div className="px-4 pb-4 space-y-3">
                  <div>
                    ...
                    <ul className="space-y-1.5">{dos.map((tip, i) => <li key={i} className="flex items-start gap-2 text-xs text-[#f5f0e8]/60"><span className="text-green-400 mt-0.5 shrink-0">{'\u2713'}</span>{tip}</li>)}</ul>
                  </div>
                  <div>
                    ...
                    <ul className="space-y-1.5">{donts.map((tip, i) => <li key={i} className="flex items-start gap-2 text-xs text-[#f5f0e8]/60"><span className="text-red-400 mt-0.5 shrink-0">{'\u2717'}</span>{tip}</li>)}</ul>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="px-4 py-3 border-t border-[#c9a84c]/10 bg-white/[0.01]">
        <p className="text-[10px] text-[#f5f0e8]/30 leading-relaxed">
          <strong className="text-[#c9a84c]/60">Pro tip:</strong> For valuable coins, consider a copy stand or smartphone macro lens attachment (~$15) for consistently sharp results.
        </p>
      </div>
    </div>
  );
