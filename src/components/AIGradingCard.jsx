        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xl font-bold" style={{ color: 'var(--cv-accent)', fontFamily: "'Playfair Display', Georgia, serif" }}>{grading.suggested_grade}</span>
              <span className="text-sm" style={{ color: 'var(--cv-text-secondary)' }}>{grading.grade_description}</span>
            </div>
            <p className="text-xs" style={{ color: 'var(--cv-text-muted)' }}>Range: {grading.estimated_grade_range}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      {expanded && (
        <div className="p-4 space-y-4">
          ...
          <div className="rounded-xl p-4" style={{ background: 'var(--cv-bg-card)', border: '1px solid var(--cv-border)' }}>
            ...
            <p className="text-sm leading-relaxed" style={{ color: 'var(--cv-text-secondary)' }}>{grading.grading_rationale}</p>
          </div>
          {grading.red_flags?.length > 0 && (
            <div className="bg-amber-500/5 rounded-xl p-4 border border-amber-500/20">
              ...
              <div className="flex flex-wrap gap-2">{grading.red_flags.map((flag, i) => <span key={i} className="text-xs bg-amber-500/10 text-amber-300 px-2 py-1 rounded-lg border border-amber-500/20">{flag}</span>)}</div>
            </div>
          )}
          ...
          <div className="pt-1">
            <button onClick={() => onAccept(grading.suggested_grade)}
              ...
              <CheckCircle className="w-4 h-4" /> {accepted ? 'Grade Accepted' : `Accept AI Grade (${grading.suggested_grade})`}
            </button>
          </div>
