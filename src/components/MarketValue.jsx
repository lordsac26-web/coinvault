...
const trendColor = { Rising: 'text-green-400', Declining: 'text-red-400', Stable: 'text-slate-400', Volatile: 'text-amber-400' };

export default function MarketValue({ coin, onUpdate }) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const data = coin.market_value;
  ...
  const fetchPrices = async () => {
    ...
    const updated = await updateCoin(coin.id, { market_value: result, market_value_at: new Date().toISOString() });
    onUpdate(updated);
    setLoading(false);
  };

  const userGrade = coin.user_grade || '';
  ...
  if (!data && !loading) {
    return (
      <div className="rounded-2xl border border-[#c9a84c]/20 p-8 text-center" style={{ background: 'rgba(255,255,255,0.02)' }}>
        ...
        <button onClick={fetchPrices} className="px-5 py-2 rounded-lg bg-gradient-to-r from-[#c9a84c] to-[#e8c97a] text-[#0a0e1a] font-semibold text-sm hover:opacity-90 transition-opacity">Fetch Market Value</button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-[#c9a84c]/30 p-6 text-center bg-gradient-to-br from-[#c9a84c]/10 to-transparent">
        <p className="text-xs text-[#f5f0e8]/40 uppercase tracking-wider mb-1">Estimated Value ({coin.user_grade})</p>
        <p className="text-4xl font-bold text-[#e8c97a]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>{data.this_coin_estimated_value || 'N/A'}</p>
        <div className="flex items-center justify-center gap-2 mt-2">
          {data.price_trend && trendIcon[data.price_trend]}
          <span className={`text-sm font-medium ${data.price_trend ? trendColor[data.price_trend] : 'text-slate-400'}`}>{data.price_trend || 'Stable'}</span>
          {data.trend_note && <span className="text-xs text-[#f5f0e8]/40">&mdash; {data.trend_note}</span>}
        </div>
                ...
                {data.retail_values.by_grade.map((row, i) => {
                  ...
                  return (
                    <tr key={i} className={`border-b border-white/5 transition-colors ${isUserGrade ? 'bg-[#c9a84c]/10' : 'hover:bg-white/[0.02]'}`}>
                      <td className={`px-4 py-2.5 font-medium ${isUserGrade ? 'text-[#e8c97a]' : 'text-[#f5f0e8]/70'}`}>{row.grade} {isUserGrade && <span className="text-xs text-[#c9a84c]">&laquo; yours</span>}</td>
      ...
      <div className="space-y-2">
        {data.sources_checked?.length > 0 && <p className="text-xs text-[#f5f0e8]/30">Sources: {data.sources_checked.join(', ')}</p>}
        <div className="flex items-start gap-1.5 text-xs text-[#f5f0e8]/30 bg-white/[0.02] rounded-lg p-3 border border-white/5">
          ...
          Prices are estimates from public sources and AI analysis. Not financial advice.
        </div>
      </div>

      <div className="flex items-center justify-between">
        {coin.market_value_at && <p className="text-xs text-[#f5f0e8]/30">Updated: {new Date(coin.market_value_at).toLocaleDateString()}</p>}
        <button onClick={fetchPrices} disabled={loading} className="flex items-center gap-1.5 text-xs text-[#c9a84c]/60 hover:text-[#c9a84c] transition-colors ml-auto">
          <RefreshCw className={`w-3 h-3 ${loading ? 'animate-spin' : ''}`} /> Update Prices
        </button>
      </div>
    </div>
  );
