  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        {/*
          FIX: StatCard icon opacity bug.
          Before: set opacity:0.12 on the wrapper div, which also faded the icon to 12%.
          After: use a semitransparent background on the inner div only, keep icon at full opacity.
          The color prop (e.g. "text-blue-400") is still used for the icon color class.
        */}
        <div
        ...
        <span className="text-xs font-medium" style={{ color: 'var(--cv-text-secondary)' }}>
          {label}
        </span>
      </div>
      <p
  ...
  return (
    ...
    >
      <Link to={`/collections/${col.id}`} className="block">
        <div
          className="h-28 sm:h-36 flex items-center justify-center overflow-hidden"
          style={{ background: `linear-gradient(135deg, var(--cv-gradient-from), var(--cv-gradient-to))` }}
        >
          {col.cover_image ? (
      ...
      <div className="px-3.5 pb-3 flex justify-end gap-2">
        ...
        <AlertDialog>
          <AlertDialogTrigger asChild>
            {/*
              FIX: Delete button mobile visibility.
              Before: md:opacity-0 md:group-hover:opacity-100 hid the button on desktop until hover,
              but on mobile there was no affordance at all.
              After: always visible at low opacity, full opacity on hover/focus.
            */}
            <button
...
export default function Dashboard() {
  ...
  // FIX: Consistent load pattern — hoisted outside useEffect so it can be called after mutations
  const load = async () => {
    setLoading(true);
  ...
  const handleCreate = async () => {
    ...
    // FIX: try/catch on all async mutations — prevents silent failures
    try {
      await createCollection(newCol);
      setNewCol({ name: '', description: '', type: 'Custom' });
      setShowCreate(false);
      // FIX: Optimistic-style update — add to local state immediately, then reload for consistency
      load();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      // FIX: Optimistic update — remove from state immediately for instant feedback
  ...
  // FIX: Memoized tag list — avoids recomputing on every render
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          ...
          >
            ...
            <div className="space-y-4 mt-2">
              ...
              />
              {/* FIX: Added visible label for collection type select for accessibility */}
              <div>
                <label
                ...
                <Select value={newCol.type} onValueChange={v => setNewCol({ ...newCol, type: v })}>
                  ...
                  >
                    {[
                      'By Country','By Type','By Era','By Series','By Metal',
                      'Proof Sets','Mint Sets','Bullion','Rolls','Commemoratives',
                      'Paper Currency','Custom',
                    ].map(t => (
                      <SelectItem key={t} value={t} style={{ color: 'var(--cv-text)' }}>
                        {t}
                      </SelectItem>
                    ))}
                  </SelectContent>
