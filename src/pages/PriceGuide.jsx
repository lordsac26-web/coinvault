...
export default function PriceGuide() {
  ...
  const handleFetch = async () => {
    if (!hasApiKey()) { setError('Add your Anthropic API key in Settings.'); return; }
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="rounded-2xl border border-[#c9a84c]/20 p-6 mb-6" style={{ background: 'rgba(255,255,255,0.02)' }}>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-5">
          <div>
            <label className="block text-xs font-medium text-[#f5f0e8]/50 uppercase tracking-wider mb-1.5">Country</label>
            <input list="pg-countries" value={form.country} onChange={e => set('country', e.target.value)}
              className="w-full bg-white/5 border border-[#c9a84c]/20 rounded-lg px-3 py-2.5 text-sm text-[#f5f0e8] focus:outline-none focus:border-[#c9a84c]/60" />
            <datalist id="pg-countries">{COUNTRIES.map(c => <option key={c} value={c} />)}</datalist>
          </div>
          <div>
            <label className="block text-xs font-medium text-[#f5f0e8]/50 uppercase tracking-wider mb-1.5">Denomination *</label>
            <input value={form.denomination} onChange={e => set('denomination', e.target.value)}
              placeholder="e.g. Morgan Dollar"
              className="w-full bg-white/5 border border-[#c9a84c]/20 rounded-lg px-3 py-2.5 text-sm text-[#f5f0e8] placeholder-[#f5f0e8]/30 focus:outline-none focus:border-[#c9a84c]/60" />
          </div>
          <div>
            <label className="block text-xs font-medium text-[#f5f0e8]/50 uppercase tracking-wider mb-1.5">Year</label>
            <input type="number" value={form.year} onChange={e => set('year', e.target.value)}
              placeholder="e.g. 1921"
              className="w-full bg-white/5 border border-[#c9a84c]/20 rounded-lg px-3 py-2.5 text-sm text-[#f5f0e8] placeholder-[#f5f0e8]/30 focus:outline-none focus:border-[#c9a84c]/60" />
          </div>
          <div>
            <label className="block text-xs font-medium text-[#f5f0e8]/50 uppercase tracking-wider mb-1.5">Mint Mark</label>
            <select value={form.mintMark} onChange={e => set('mintMark', e.target.value)}
              className="w-full bg-[#111827] border border-[#c9a84c]/20 rounded-lg px-3 py-2.5 text-sm text-[#f5f0e8] focus:outline-none">
              {['None', 'P', 'D', 'S', 'O', 'CC', 'W'].map(m => <option key={m} value={m}>{m}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-[#f5f0e8]/50 uppercase tracking-wider mb-1.5">Your Grade</label>
            <select value={form.userGrade} onChange={e => set('userGrade', e.target.value)}
              className="w-full bg-[#111827] border border-[#c9a84c]/20 rounded-lg px-3 py-2.5 text-sm text-[#f5f0e8] focus:outline-none">
              {SHELDON_GRADES.map(g => <option key={g.grade} value={g.grade}>{g.grade}</option>)}
            </select>
          </div>
          <div className="flex items-end">
            <button onClick={handleFetch} disabled={loading}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-gradient-to-r from-[#c9a84c] to-[#e8c97a] text-[#0a0e1a] font-semibold text-sm hover:opacity-90 disabled:opacity-50 transition-opacity">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-[#c9a84c]/30 p-6 text-center bg-gradient-to-br from-[#c9a84c]/10 to-transparent">
            ...
            <p className="text-4xl font-bold text-[#e8c97a]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
              {result.this_coin_estimated_value || result.thisCoinsEstimatedValue || 'N/A'}
            </p>
          </div>
                  ...
                  <tbody>
                    {result.retail_values.by_grade.map((row, i) => (
                      <tr key={i} className={`border-b border-white/5 ${row.grade === form.userGrade ? 'bg-[#c9a84c]/10' : 'hover:bg-white/[0.02]'}`}>
                        <td className={`px-4 py-2.5 font-medium ${row.grade === form.userGrade ? 'text-[#e8c97a]' : 'text-[#f5f0e8]/70'}`}>
                          {row.grade} {row.grade === form.userGrade && <span className="text-xs text-[#c9a84c]">◀</span>}
                        </td>
