...
import { ArrowLeft, Plus, Grid3X3, List, BookOpen, Search, Loader2 } from 'lucide-react';
import { getCollections, getCoinsByCollection } from '../lib/storage';
...
const CoinCircle = ({ coin, onClick }) => (
  <div className="group relative cursor-pointer" onClick={onClick}
    ...
    <div className="w-full aspect-square rounded-full overflow-hidden relative"
      style={{ border: '2px solid rgba(201,168,76,0.4)', boxShadow: '0 4px 16px rgba(0,0,0,0.4), 0 0 0 1px rgba(201,168,76,0.1)', background: 'rgba(255,255,255,0.03)' }}>
      {coin.obverse_image ? (
        <img src={coin.obverse_image} alt="" className="w-full h-full object-cover" />
      ) : (
        <div className="w-full h-full flex items-center justify-center">
          <div className="text-center">
            <p className="text-xs font-bold text-[#e8c97a]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>{coin.year || '?'}</p>
      ...
      <div className="absolute bottom-1 left-0 right-0 flex justify-center opacity-0 group-hover:opacity-100 transition-opacity">
        <GradeBadge grade={coin.user_grade} />
      </div>
    </div>
    <p className="text-center text-[10px] text-[#f5f0e8]/40 mt-1.5 truncate">{coin.year} {coin.mint_mark !== 'None' ? coin.mint_mark : ''}</p>
  </div>
);
...
export default function CollectionView() {
  ...
  const load = async () => {
    setLoading(true);
    const [cols, coinList] = await Promise.all([
      getCollections(),
      getCoinsByCollection(id),
    ]);
  ...
  if (!collection) return (
    <div className="max-w-7xl mx-auto px-6 py-16 text-center">
      ...
      <Link to="/collections" className="text-[#e8c97a] text-sm mt-2 inline-block hover:underline">&larr; Back to Collections</Link>
    </div>
  );

  const filtered = coins
    .filter(c => !search || [c.denomination, c.year, c.country, c.mint_mark, c.user_grade].some(v => v?.toLowerCase().includes(search.toLowerCase())))
    .sort((a, b) => {
      if (sortBy === 'year') return parseInt(a.year) - parseInt(b.year);
      if (sortBy === 'grade') return (b.user_grade || '').localeCompare(a.user_grade || '');
      if (sortBy === 'value') return (parseFloat(b.purchase_price) || 0) - (parseFloat(a.purchase_price) || 0);
      return new Date(b.created_date) - new Date(a.created_date);
    });

  const totalValue = coins.reduce((s, c) => s + (parseFloat(c.purchase_price) || 0), 0);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-[#f5f0e8]" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>{collection.name}</h1>
        ...
        {collection.target_goal && (
          <div className="mt-3 p-3 rounded-xl border border-[#c9a84c]/20 bg-[#c9a84c]/5">
            <p className="text-xs text-[#c9a84c] mb-1.5">Goal: {collection.target_goal}</p>
      ...
      {coins.length === 0 && (
        <div className="rounded-2xl border border-[#c9a84c]/20 p-16 text-center" style={{ background: 'rgba(255,255,255,0.02)' }}>
          ...
          <h3 className="text-lg font-semibold text-[#f5f0e8]/50 mb-2" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
            Your collection awaits &mdash; add your first coin
          </h3>
            ...
            <tbody>
              {filtered.map(coin => (
                <tr key={coin.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors cursor-pointer"
                  ...
                  <td className="px-4 py-3 text-[#f5f0e8]/60">{coin.mint_mark !== 'None' ? coin.mint_mark : '\u2014'}</td>
                  <td className="px-4 py-3 text-[#f5f0e8]/70">{coin.denomination}</td>
                  <td className="px-4 py-3"><GradeBadge grade={coin.user_grade} /></td>
                  <td className="px-4 py-3 text-[#f5f0e8]/60">{coin.purchase_price ? `$${coin.purchase_price}` : '\u2014'}</td>
                  <td className="px-4 py-3 text-[#f5f0e8]/40">{new Date(coin.created_date).toLocaleDateString()}</td>
                  <td className="px-4 py-3"><button className="text-xs text-[#c9a84c]/60 hover:text-[#c9a84c]">View &rarr;</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {viewMode === 'binder' && (
        <div className="rounded-2xl border border-[#c9a84c]/20 p-8" style={{ background: 'rgba(10,15,30,0.8)' }}>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-base font-semibold text-[#f5f0e8]/60" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>{collection.name} &mdash; Binder View</h3>
          </div>
      ...
      {showAddCoin && (
        <AddCoinWizard collectionId={id} onClose={() => setShowAddCoin(false)} onAdded={() => { setShowAddCoin(false); load(); }} />
      )}
    </div>
  );
}